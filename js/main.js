...
$("#date-slider").slider({
    ...
    min: parseTime("27/3/2017").getTime(),
...
function getData(stock){
    let code;
    if (stock==="apple") code='AAPL';
    if (stock==="facebook") code='FB';
    let c = window.filteredData;

    d3.json(`https://www.quandl.com/api/v3/datasets/WIKI/${code}.json?start_date=2017-03-27&api_key=wzN4rh3biaLFaksci2K4`).then(function(data){
        c[stock] = [];
        data.dataset.data.forEach(function(currentData){
                date = parseTime(formatTime(new Date(currentData[0])));
                c[stock].push(
                    {
                        open: currentData[1],
                        high: currentData[2],
        ...
        window.filteredData = c;
        completeUpdate(stock);
...
function update() {
    ...
    var coin = $("#coin-select").val();
        window.yValue = $("#var-select").val();
        window.sliderValues = $("#date-slider").slider("values");
    if (!window.filteredData){
        window.filteredData = {};
    }
    if (!window.filteredData[coin]){
        getData(coin)
    }
    else {
        completeUpdate(coin);
...
function completeUpdate(coin){

    var dataTimeFiltered = filteredData[coin].filter(function(d){
        return ((d.date >= sliderValues[0]) && (d.date <= sliderValues[1]))
    });
    dataTimeFiltered.sort(function(a, b) { return a.date - b.date; });
    ...
    // Path generator
    line = d3.line()
        .x(function(d){ console.log(yValue); return x(d.date); })
